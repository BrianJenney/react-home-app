...
class AddProp extends React.Component {
	constructor(props) {
		...
		this.years = Array.from(new Array(70), (val, index) => currentYear - index);
	}
	...
	postHome = (form) => {
		API.posthome(this.state.form)
			.then((response) => {
				if (response.errors) {
					alert(`Oops, something went wrong. Try uploading your property again.`);
				}
		...
		return (
			<div>
				...
				<div className='saveDraft-publish-buttons'>
					...
					<button id='pubButton' onClick={this.submitProperty.bind(this, 'publish')}>
						Publish Listing
					</button>
				</div>
				...
				<div className='pic-preview ml-4'>
					<PicPreview pics={this.state.imgs} removePic={this.removePic} />
				</div>
				...
				<div className='container-fluid'>
					<div className='edit-property-details mt-2'>
						<div className='form-inline'>
							<select id='bedRooms' value={this.state.bedRooms} onChange={this.onChange.bind(this)}>
						...
						<div className='form-inline'>
							<select value={this.state.bathRooms} id='bathRooms' onChange={this.onChange.bind(this)}>
								<option value='1'>1</option>
								<option value='2'>2</option>
								<option value='3'>3</option>
								<option value='4'>4</option>
						...
						<div className='form-inline'>
							<select
								...
								onChange={this.onChange.bind(this)}
							>
								<option value='Single Family Home'>Singe Family Home</option>
						...
						<div className='form-inline'>
							<select value={this.state.yearBuilt} id='yearBuilt' onChange={this.onChange.bind(this)}>
...
export default GoogleApiHOC(AddProp);
