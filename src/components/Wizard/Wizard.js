...
const Quiz = ({ optionChangeHandlerCallback, total, completeWizard, config }) => {
	...
	return (
		<div className='wizard-body'>
			...
			<Question {...config[page]} optionChangeHandler={optionChangeHandler} />
			<div className='progress-bar-container'>
				{page > 0 ? (
					<i onClick={() => changePageBackward} className='material-icons' style={{ color: 'white' }}>
						arrow_back
					</i>
				) : null}
				...
				<i onClick={() => changePageForward()} className='material-icons'>
					arrow_forward
				</i>
			</div>
		</div>
